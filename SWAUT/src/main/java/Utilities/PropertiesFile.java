...
extends DataDriver
{
	...
	public static void createConfigFile(){
		...
		try
		{
			DataDriver.useExcelSheet("./src/test/resources/AppConfiguration.xls", 1);
	...
	/*public static void setAppConfig(){
		DataDriver.useExcelSheet(Common.getConfigProperty("Tc_Settings_Excelpath"), 1);
		Sheet readAppSheet = w.getSheet(0);
		for (int j = 1; j < readAppSheet.getRows(); j++){
			String strOS = readAppSheet.getCell(1, j).getContents();
			if(!(strOS.equals(""))){
				String strExecutionEnv = readAppSheet.getCell(1, j).getContents();
				String strHost = readAppSheet.getCell(2, j).getContents();
				String strPort = readAppSheet.getCell(3, j).getContents();
				String strBrowser = readAppSheet.getCell(4, j).getContents();				
				String strUrl = readAppSheet.getCell(6, j).getContents();				
				Common.setConfigProperty("ExecEnv",strExecutionEnv);
				Common.setConfigProperty("url",strUrl);				
				Common.setConfigProperty("Host",strHost);		
				Common.setConfigProperty("Port",strPort);
				Common.setConfigProperty("Browser",strBrowser);
			}		  
		}
